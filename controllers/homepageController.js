...
const getFeaturedTestimonials = async (req, res) => {
  try {
    const { limit = 6, product_id } = req.query;
    ...
    // If product_id is provided, filter by it; otherwise get featured testimonials
    if (product_id) {
    ...
    const testimonials = await Testimonial.findAll({
      ...
      attributes: ['id', 'name', 'rating', 'review', 'image_url', 'location', 'is_verified', 'product_id']
    });
